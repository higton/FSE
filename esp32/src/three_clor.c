...
void rgb_led_set_red(uint8_t value){ // set the red LED
    data->red = value;
...
void rgb_led_set_green(uint8_t value){  // set the green LED
    data->green = value;
    if(!g_pwm_init_handle){rgb_led_pwm_init();}
    ledc_set_duty(ledc_ch[1].mode, ledc_ch[1].channel, data->green);
	ledc_update_duty(ledc_ch[1].mode, ledc_ch[1].channel);
...
void rgb_led_set_blue(uint8_t value){ // set the blue LED
    data->blue = value;
    if(!g_pwm_init_handle){rgb_led_pwm_init();}
    ledc_set_duty(ledc_ch[2].mode, ledc_ch[2].channel, data->blue);
	ledc_update_duty(ledc_ch[2].mode, ledc_ch[2].channel);
...
int rgb_led_get_red(){return data->red;}
int rgb_led_get_green(){return data->green;}
int rgb_led_get_blue(){return data->blue;}
